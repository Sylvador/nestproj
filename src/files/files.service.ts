...
export class FilesService {
  ...
  async createFile(file: Express.Multer.File, fileDto: FileDto): Promise<string> {
    try {
  ...
  async bindImageToInstance(fileDto: FileDto): Promise<Files> {
  ...
  async addImageAsPreview(file: Express.Multer.File): Promise<string> {
    const fileName = uuid.v4() + '.jpg';
